...
class CA:
    ...
    def makeframes(self, current, patlist, positions, bbox, pad):
    ...
    def makegif(self, current, gen, step):
        ...
        duration = max(1/60, (-0.5 / (-gen // 10)) / step) if gen else 1
        try:
    ...
    def makesoup(self, rulestring, x, y):
    ...
    async def sim(self, ctx, gen: int, step: int=1, rule='B3/S23', pat=None, **kwargs):
        ...
        patlist, positions, bbox = await self.loop.run_in_executor(self.executor, self.parse, current)
        await self.loop.run_in_executor(self.executor, self.makeframes, current, patlist, positions, bbox, len(str(gen)))
        try:
            await self.loop.run_in_executor(self.executor, self.makegif, current, gen, step)
